/*
 * Copyright (c) 2018, 2020-2022, Jan-Gerd Mess
...
public:
    ...
    static_assert(Counter::is_always_lock_free, "Reference counter is not lock free");

    /**
     * \brief Constructor for an empty (invalid) buffer.
     *
     * Needed for array creation. The array then needs to be filled with
     * \c setPointer(uint8_t*,size_t) or \c SharedBuffer::setData
     */
    SharedBuffer();

    /**
     * \brief Constructor for creating a SharedBuffer instance from a pointer
     *      and a given size in bytes.
     *
     * \param slice
     *      Slice holding the byte array.
     */
     ...
     *
     * Should only be called manually if absolutely certain that the buffer is
     * not in use or part of a pool.
     */
     ...
     *
     * \param index
     *      Index to be returned. Warning: Any checks for valid index ranges
    ...
    /**
     * \brief Setter function the pointer and size of an unused SharedBuffer.
     *      Nothing will happen, if the buffer is currently in use.
     *
     * \param slice
     *      Slice holding the new byte buffer.
     */
    inline void
    ...
    /**
     * \brief Getter function for the usage state of the \c SharedBuffer.
     *      Calls \c outpost::rtos::Mutex protected functions internally.
     *
     * \retval true, if the SharedBuffer is currently in use
     * \retval false, otherwise.
     */
    inline bool
    isUsed() const
    {
        return mReferenceCounter.load(std::memory_order_acquire) != 0;
    }
     ...
     *
     * \retval true, if the underlying byte array is not a nullptr
     * \retval false, otherwise
     */
    inline bool
    isValid() const
...
protected:
     ...
     */
    SharedBufferPointerBase(SharedBufferPointerBase&& other) :
        mPtr(other.mPtr), mType(other.mType), mOffset(other.mOffset), mLength(other.mLength)
    {
        other.reset();
    }
     ...
     */
    SharedBufferPointerBase&
    operator=(SharedBufferPointerBase&& other)
    {
        if (&other != this)
        {
            decrementCount();
    ...
    void
    copy(const SharedBufferPointerBase& other)
    {
        mPtr = other.mPtr;
        mType = other.mType;
    ...
    void
    copyAndReset(SharedBufferPointerBase& other)
    {
        copy(other);
        other.reset();
    }
...
public:
     ...
     * \param other Reference of the SharedBufferPointer instance to be moved.
     */
    SharedBufferPointer(SharedBufferPointer&& other) : SharedBufferPointerBase(std::move(other))
    {
    }
     ...
     */
    SharedBufferPointer&
    operator=(SharedBufferPointer&& other)
    {
        SharedBufferPointerBase::operator=(std::move(other));
        return *this;
    }
...
public:
     ...
     */
    // cppcheck-suppress noExplicitConstructor
    ConstSharedBufferPointer(SharedBufferPointer&& other) :
        SharedBufferPointerBase(std::move(other))
    {
    }
     ...
     */
    ConstSharedBufferPointer(ConstSharedBufferPointer&& other) :
        SharedBufferPointerBase(std::move(other))
    {
    }
     ...
     */
    ConstSharedBufferPointer&
    operator=(ConstSharedBufferPointer&& other)
    {
        SharedBufferPointerBase::operator=(std::move(other));
        return *this;
    }
...
public:
     ...
     */
    SharedChildPointer(SharedChildPointer&& other) :
        SharedBufferPointer(std::move(static_cast<SharedBufferPointer&&>(other))),
...
public:
     ...
     */
    // cppcheck-suppress noExplicitConstructor
    ConstSharedChildPointer(SharedChildPointer&& other) :
        ConstSharedBufferPointer(std::move(static_cast<ConstSharedBufferPointer&&>(other))),
        mParent(std::move(other.mParent))
    {
    }
     ...
     */
    ConstSharedChildPointer(ConstSharedChildPointer&& other) :
        ConstSharedBufferPointer(std::move(static_cast<ConstSharedBufferPointer&&>(other))),
        mParent(std::move(other.mParent))
    {
    }
     ...
     */
    ConstSharedChildPointer&
    operator=(ConstSharedChildPointer&& other)
    {
        if (&other != this)
        {
            decrementCount();
