...
public:
    ...
    /**
     * Pushes a single Fixedpoint to the current DataBlock, outputs completed DataBlocks and starts
     * a new one if needed.
     * @param fp Fixedpoint number to be added to a DataBlock
     * @param currentTime Current GpsTime of the data sample
     * @return Returns true if the Fixedpoint number could be stored in a DataBlock, false
     * otherwise.
     */
    bool
    push(Fixedpoint fp, const outpost::time::GpsTime& currentTime);
...
protected:
    ...
    outpost::container::SharedBufferPoolBase* mMemoryPool;
