...
public:
    ...
    outpost::container::SharedBufferPool<16384, 20> mPool;
};
...
TEST_F(DataBlockTest, Push)
{
    ...
    {
        EXPECT_FALSE(block.isComplete());
        EXPECT_TRUE(block.push(Fixedpoint(i)));
        EXPECT_EQ(block.getSampleCount(), i + 1);
        EXPECT_TRUE(block.isValid());
    }
    EXPECT_TRUE(block.isComplete());
    EXPECT_FALSE(block.push(Fixedpoint(123)));
    EXPECT_EQ(block.getSampleCount(), 16U);
    EXPECT_TRUE(block.isValid());

    outpost::Slice<Fixedpoint> slice = block.getSamples();
    EXPECT_EQ(slice.getNumberOfElements(), 16U);

    for (int32_t i = 0; i < 16; i++)
    {
        EXPECT_EQ(slice[i], Fixedpoint(i));
    }
}
...
TEST_F(DataBlockTest, GetSamples)
{
    ...
    {
        block.push(Fixedpoint(i));
    }

    outpost::Slice<Fixedpoint> samples = block.getSamples();
    EXPECT_EQ(samples.getNumberOfElements(), 16U);

    for (int16_t i = 0; i < 16; i++)
    {
        EXPECT_EQ(samples[i], Fixedpoint(i));
    }
...
TEST_F(DataBlockTest, Encode)
{
    ...
    outpost::container::SharedBufferPointer p_out;
