...
public:
    ...
    outpost::container::SharedBufferPool<16500, 10> mPool;
    outpost::container::ReferenceQueue<DataBlock, 8> mQueue;
...
TEST_F(DataAggregationTest, PushOnce)
{
    ...
    DataBlock b;
    EXPECT_FALSE(mQueue.receive(outpost::time::Duration::zero()));
...
TEST_F(DataAggregationTest, PushBlock)
{
    ...
    EXPECT_TRUE(aggregator.push(f, now));
    auto res = mQueue.receive(outpost::time::Duration::zero());
    ASSERT_TRUE(res);
    b = *res;
...
TEST_F(DataAggregationTest, PushBlock4096)
{
    ...
    {
        Fixedpoint fadd(static_cast<int>(i));
        fadd = f + fadd;
...
TEST_F(DataAggregationTest, ChangeBlocksize)
{
    ...
    res = mQueue.receive(outpost::time::Duration::zero());
    ASSERT_TRUE(res);
    b = *res;
...
{
    outpost::container::ReferenceQueue<DataBlock, 12> q;
    ...
    {
        EXPECT_TRUE(q.receive(outpost::time::Duration::zero()));
    }
...
{
    outpost::container::ReferenceQueue<DataBlock, 1> q;
    OneTimeQueueSender ots(q);
    DataAggregator aggregator(123U, mPool, ots);
