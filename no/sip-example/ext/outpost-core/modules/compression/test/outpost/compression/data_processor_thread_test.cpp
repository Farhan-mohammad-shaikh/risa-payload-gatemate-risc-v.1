...
public:
    ...
    outpost::container::SharedBufferPool<16500, 20> mPool;
    outpost::container::ReferenceQueue<DataBlock, 8> mInputQueue;
    outpost::container::ReferenceQueue<DataBlock, 8> mOutputQueue;
...
TEST_F(DataProcessorThreadTest, processSingleInvalidBlock)
{
    ...
    EXPECT_FALSE(mOutputQueue.receive(outpost::time::Duration::zero()));
}
...
TEST_F(DataProcessorThreadTest, processSingleBlock)
{
    ...
    {
        block.push(outpost::Fixedpoint(i));
    }
    mInputQueue.send(block);
    ...
    auto res = mOutputQueue.receive(outpost::time::Duration::zero());
    ASSERT_TRUE(res);
    block = *res;
...
TEST_F(DataProcessorThreadTest, processMultipleBlocks)
{
    ...
    {
        EXPECT_TRUE(mOutputQueue.receive());
    }
    EXPECT_FALSE(mOutputQueue.receive(outpost::time::Duration::zero()));
