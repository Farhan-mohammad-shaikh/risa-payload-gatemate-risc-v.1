...
RmapPacket::RmapPacket(Slice<uint8_t> spwTargets,
...

{
    ...
    {
        // FIXME: replyAddress is not boundschecked!
...
void
RmapPacket::setData(const Slice<const uint8_t>& data)
{
    mData = data;
    mDataLength = data.getNumberOfElements();
    mDataCRC = Crc8CcittReversed::calculate(data);
}

bool
RmapPacket::constructPacket(Slice<uint8_t>& buffer)
{
...
RmapPacket::extractReplyPacket(const Slice<const uint8_t>& data,
                               const uint8_t expectedInitiatorLogicalAddress)
{
    ...
    const uint8_t packetInitiatorLogicalAddress = stream.read<uint8_t>();

    if (packetInitiatorLogicalAddress != expectedInitiatorLogicalAddress)
    {
    ...
    if (isReplyPacket())
    {
        ...
        memset(mReplyAddress, 0, rmap::maxNumberOfTargetNodes);

        mInitiatorLogicalAddress = packetInitiatorLogicalAddress;
        ...
        if (isWrite())
        {
            ...
            const auto crcSlice = stream.asSlice();
            const uint8_t packetHeaderCRC = stream.read<uint8_t>();
            const uint8_t calculatedHeaderCRC = Crc8CcittReversed::calculate(crcSlice);
        ...
        else
        {
            ...
            const uint8_t calculatedDataCRC = Crc8CcittReversed::calculate(mData);
...
RmapPacket::setTargetInformation(RmapTargetNode& rmapTargetNode)
{
    ...
    Slice<uint8_t> rplyAddr = rmapTargetNode.getReplyAddress();
    if (rplyAddr.getNumberOfElements() > 0)
    {
        memcpy(mReplyAddress, rplyAddr.begin(), rplyAddr.getNumberOfElements());
    }
...
void
RmapPacket::constructHeader(Serialize& stream)
{
    ...
    if (isCommandPacket())
    {
        ...
        stream.store24(mDataLength);  // unsigned
    }

    mHeaderLength = stream.getPosition();
    const auto crcSlice = stream.asSlice()
                                  .skipFirst(mSpwTargetAddressLength)
                                  .first(mHeaderLength - mSpwTargetAddressLength);

    mHeaderCRC = Crc8CcittReversed::calculate(crcSlice);
    stream.store(mHeaderCRC);
}
