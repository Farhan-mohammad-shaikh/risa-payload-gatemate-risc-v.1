...
public:
    ...
    ParameterBase(ParameterList& list = IndexedParameterList<0>::getList()) :
        ...
        mWritesCounter(0)
    {
        // in C++17 this check can be a static_assertion
        // if this assertion fails a mutex is used to implement
        // the atomic
        OUTPOST_ASSERT(mWritesCounter.is_lock_free(),
                       "The libc++ does not support lock free counter!");
...
protected:
    using CounterRaw = uint32_t;
    using Counter = std::atomic<CounterRaw>;
    ...
    mutable Counter mWritesCounter;
