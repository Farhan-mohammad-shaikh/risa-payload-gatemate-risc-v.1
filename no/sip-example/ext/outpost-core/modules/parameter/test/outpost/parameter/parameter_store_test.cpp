...
TEST(ParameterStoreTest, duplicatedID)
{
    ParameterList list;
    outpost::time::SpacecraftElapsedTime time;
    ...
    EXPECT_EQ(OperationResult::duplicatedID, store.initialize(list));
    EXPECT_EQ(false, store.isInitialized());
}

TEST(ParameterStoreTest, dublicatedIDOldspellingBackwardsCompatibility)
{
    EXPECT_EQ(OperationResult::duplicatedID, OperationResult::dublicatedID);
}
...
TEST(ParameterStoreTest, dublicatedIDMultiList)
{
    ...
    EXPECT_EQ(OperationResult::duplicatedID, store.initialize(outpost::asSlice(array)));
    EXPECT_EQ(false, store.isInitialized());
}
...
TEST(ParameterStoreTest, getValue)
{
    ...
    // add them unsorted for additional testing
    constexpr IDType id[5] = {ParameterBase::invalidID + 10,
                              ParameterBase::invalidID + 9,
                              ParameterBase::invalidID + 12,
                              ParameterBase::invalidID + 13,
                              ParameterBase::invalidID + 8};
    constexpr uint32_t data[5] = {10, 14, 17, 19, 20};
    for (int i = 0; i < 5; i++)
    {
