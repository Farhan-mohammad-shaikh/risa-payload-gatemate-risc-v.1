 ...
 * Copyright (c) 2015, 2018, Olaf Maibaum
 * Copyright (c) 2019-2020, Jan Malburg
 * Copyright (c) 2022, Tobias Pfeffer
 * Copyright (c) 2022-2023, Passenberg, Felix Constantin
...
{
public:
    explicit inline Serialize(outpost::Slice<uint8_t> array) :
        mBuffer(array.begin()), mBegin(array.begin())
    {
    }
    ...
    template <typename U>
    inline void
    // False positive: Calls non-static functions, so cannot be static itself.
...
public:
    ...
    read(outpost::Slice<uint8_t> array)
    {
        ...
        if (0 < length)
        {
            memcpy(array.getDataPointer(), mBuffer, length);
            mBuffer += length;
        }
    }
