...
private:
    ...
    constexpr uint32_t
    berMas(std::array<int32_t, mTParam + 1>& sigmaN, bool& success);

    constexpr bool
    chienSearch(std::array<int32_t, mTParam + 1>& sigmaN, const int32_t LnOrig);

    constexpr bool
    quadraticElp(std::array<int32_t, mTParam + 1> const& sigmaN);

    constexpr bool
    cubicElp(std::array<int32_t, mTParam + 1> const& sigmaN);

    constexpr bool
    quarticElp(std::array<int32_t, mTParam + 1>& sigmaN);

    constexpr bool
    rootFindChien(std::array<int32_t, mTParam + 1>& sigmaN, const int32_t LnOrig);
