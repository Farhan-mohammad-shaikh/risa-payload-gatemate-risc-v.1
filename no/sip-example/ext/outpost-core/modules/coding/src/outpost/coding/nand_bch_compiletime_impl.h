    ...
    {  // alpha 1,3,5,7 etc.
        ...
        {  // If this root not already processed
            ...
            {  // Infinite loop - Exit is by "break"
                ...
                int32_t tmp[mTParam * mMParam + 1] = {0};
...
NandBCHCTime<mMParam, mTParam, mNandDataSize, mNandSpareSize>::bchEncode(void)
{
    ...
    //****************************************************************
    std::array<uint32_t, mNumRedundantWords> SR = {};
    ...
    std::array<int32_t, (mTParam * mMParam) / 8 + 5> redunByteArray = {};
...
template <uint32_t mMParam, uint32_t mTParam, uint32_t mNandDataSize, uint32_t mNandSpareSize>
constexpr uint32_t
NandBCHCTime<mMParam, mTParam, mNandDataSize, mNandSpareSize>::berMas(
        std::array<int32_t, mTParam + 1>& sigmaN, bool& success)
{
    //***************************************************************
    //  Function: BerMas
    //
    ...
    //***************************************************************
    std::array<int32_t, mTParam + 1> sigmaK;
...
template <uint32_t mMParam, uint32_t mTParam, uint32_t mNandDataSize, uint32_t mNandSpareSize>
constexpr bool
NandBCHCTime<mMParam, mTParam, mNandDataSize, mNandSpareSize>::chienSearch(
        std::array<int32_t, mTParam + 1>& sigmaN, const int32_t LnOrig)
{
    //****************************************************************
...
template <uint32_t mMParam, uint32_t mTParam, uint32_t mNandDataSize, uint32_t mNandSpareSize>
constexpr bool
NandBCHCTime<mMParam, mTParam, mNandDataSize, mNandSpareSize>::quadraticElp(
        std::array<int32_t, mTParam + 1> const& sigmaN)
{
    //****************************************************************
...
template <uint32_t mMParam, uint32_t mTParam, uint32_t mNandDataSize, uint32_t mNandSpareSize>
constexpr bool
NandBCHCTime<mMParam, mTParam, mNandDataSize, mNandSpareSize>::cubicElp(
        std::array<int32_t, mTParam + 1> const& sigmaN)
{
    //****************************************************************
    //  Function:   cubicElp
...
template <uint32_t mMParam, uint32_t mTParam, uint32_t mNandDataSize, uint32_t mNandSpareSize>
constexpr bool
NandBCHCTime<mMParam, mTParam, mNandDataSize, mNandSpareSize>::quarticElp(
        std::array<int32_t, mTParam + 1>& sigmaN)
{
    //****************************************************************
    //  Function:   quarticElp
    //
    ...
    else
    {
        ...
        int32_t b4n = ffMult(sigbk[1], sigbk[1]);
        int32_t b4d = ffMult(sigbk[3], sigbk[3]) ^ ffMult(sigbk[1], ffMult(sigbk[2], sigbk[3]))
                      ^ ffMult(sigbk[4], b4n);
...
template <uint32_t mMParam, uint32_t mTParam, uint32_t mNandDataSize, uint32_t mNandSpareSize>
constexpr bool
NandBCHCTime<mMParam, mTParam, mNandDataSize, mNandSpareSize>::rootFindChien(
        std::array<int32_t, mTParam + 1>& sigmaN, const int32_t LnOrig)
{
    //****************************************************************
...
NandBCHCTime<mMParam, mTParam, mNandDataSize, mNandSpareSize>::decode(
        const outpost::Slice<const uint8_t>& coded_data, const outpost::Slice<uint8_t>& dst_data)
{
    if (coded_data.getNumberOfElements() < mNandDataSize + mNandSpareSize)
    {
        // too small data or invalid template parameter
    ...
    for (uint32_t i = 0; i < iteration_count; i++)
    {
        ...
        if (dst_data.getNumberOfElements() >= ((i + 1) * mNumDataBytes))
        {
            // all data fitting
            memcpy(&dst_data[0] + (i * mNumDataBytes), mCodeWord, mNumDataBytes);
        }
        else if (dst_data.getNumberOfElements() <= (i * mNumDataBytes))
        {
        ...
        {
            // partial
            uint32_t sizeRemaining = dst_data.getNumberOfElements() - (i * mNumDataBytes);
            memcpy(&dst_data[0] + (i * mNumDataBytes), mCodeWord, sizeRemaining);
        }
    }
    return status;
}
