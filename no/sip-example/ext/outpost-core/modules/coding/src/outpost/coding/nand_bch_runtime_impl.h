    ...
    encodeTable{}
{
    ...
    if (!generateCodeGenPoly())
    {
        mValid = false;
        return;
    }

    mNumRedundantBits = genPolyDegree;
    mNumRedundantBytes = genPolyDegree / 8;

    if ((genPolyDegree % 8) > 0)
        mNumRedundantBytes++;
...
template <uint32_t mMParam, uint32_t mTParam, uint32_t mNandDataSize, uint32_t mNandSpareSize>
void
NandBCHRTime<mMParam, mTParam, mNandDataSize, mNandSpareSize>::buildLogTables(void)
{
    ...
    {
        aLogTable[i] = static_cast<int32_t>(shiftReg);
        if (i < mNParam)
            logTable[shiftReg] = i;
    ...
    logTable[0] = mLogZVal;  // log of zero
    aLogTable[mLogZVal] = 0;
}
...
template <uint32_t mMParam, uint32_t mTParam, uint32_t mNandDataSize, uint32_t mNandSpareSize>
void
NandBCHRTime<mMParam, mTParam, mNandDataSize, mNandSpareSize>::genTraceTestVal(void)
{
    mTraceTestVal = 0;
    uint32_t shifter = 1;
...
template <uint32_t mMParam, uint32_t mTParam, uint32_t mNandDataSize, uint32_t mNandSpareSize>
void
NandBCHRTime<mMParam, mTParam, mNandDataSize, mNandSpareSize>::genQuadCompTable(void)
{
    //****************************************************************
    ...
    for (uint32_t kx = 0; kx < mMParam; kx++)
    {
        mQuadCompTable[kx] = 0;  // Clear table
    }
...
template <uint32_t mMParam, uint32_t mTParam, uint32_t mNandDataSize, uint32_t mNandSpareSize>
void
NandBCHRTime<mMParam, mTParam, mNandDataSize, mNandSpareSize>::bchInit(void)
{
    buildLogTables();
    genTraceTestVal();
    genQuadCompTable();
...
template <uint32_t mMParam, uint32_t mTParam, uint32_t mNandDataSize, uint32_t mNandSpareSize>
bool
NandBCHRTime<mMParam, mTParam, mNandDataSize, mNandSpareSize>::generateCodeGenPoly(void)
{
    //****************************************************************
    ...
    {  // alpha 1,3,5,7 etc.
        ...
        {  // If this root not already processed
            ...
            {  // Infinite loop - Exit is by "break"
                ...
                if (genPolyDegree + 1 > mMParam * mTParam)
                {
                ...
                tmp[0] = 0;
                genPolyDegree++;
                for (int32_t kx = genPolyDegree; kx >= 0; kx--)
                {
...
template <uint32_t mMParam, uint32_t mTParam, uint32_t mNandDataSize, uint32_t mNandSpareSize>
void
NandBCHRTime<mMParam, mTParam, mNandDataSize, mNandSpareSize>::convertGenPolyBitToWord(void)
{
    //****************************************************************
    ...
    for (int32_t kkk = mNumRedundantBits - 1; kkk >= 0; kkk--)
    {
        if (genPolyBitArray[kkk] > 0)
        {
            genPolyFdbkWords[wordAddr] ^= bitMask;
        }
...
template <uint32_t mMParam, uint32_t mTParam, uint32_t mNandDataSize, uint32_t mNandSpareSize>
void
NandBCHRTime<mMParam, mTParam, mNandDataSize, mNandSpareSize>::generateEncodeTables(void)
{
    //****************************************************************
    ...
    {  // Encoding is 8 bits parallel
        ...
        for (uint32_t k = 0; k < mNumRedundantWords; k++)
        {
            encodeTable[i][k] = SR[k];  // Move SR to encode table
        }
    }
}
...
template <uint32_t mMParam, uint32_t mTParam, uint32_t mNandDataSize, uint32_t mNandSpareSize>
void
NandBCHRTime<mMParam, mTParam, mNandDataSize, mNandSpareSize>::bchEncode(void)
{
    //****************************************************************
    //  Function: bchEncode
    //
...
template <uint32_t mMParam, uint32_t mTParam, uint32_t mNandDataSize, uint32_t mNandSpareSize>
int32_t
NandBCHRTime<mMParam, mTParam, mNandDataSize, mNandSpareSize>::computeRemainder(void)
{
    //****************************************************************
...
template <uint32_t mMParam, uint32_t mTParam, uint32_t mNandDataSize, uint32_t mNandSpareSize>
void
NandBCHRTime<mMParam, mTParam, mNandDataSize, mNandSpareSize>::computeSyndromes(void)
{
    //****************************************************************
...
template <uint32_t mMParam, uint32_t mTParam, uint32_t mNandDataSize, uint32_t mNandSpareSize>
bool
NandBCHRTime<mMParam, mTParam, mNandDataSize, mNandSpareSize>::chienSearch(int32_t sigmaN[],
...
template <uint32_t mMParam, uint32_t mTParam, uint32_t mNandDataSize, uint32_t mNandSpareSize>
bool
NandBCHRTime<mMParam, mTParam, mNandDataSize, mNandSpareSize>::quadraticElp(const int32_t sigmaN[])
{
    //****************************************************************
...
template <uint32_t mMParam, uint32_t mTParam, uint32_t mNandDataSize, uint32_t mNandSpareSize>
bool
NandBCHRTime<mMParam, mTParam, mNandDataSize, mNandSpareSize>::cubicElp(const int32_t sigmaN[])
{
    //****************************************************************
    //  Function:   cubicElp
    //
...
template <uint32_t mMParam, uint32_t mTParam, uint32_t mNandDataSize, uint32_t mNandSpareSize>
bool
NandBCHRTime<mMParam, mTParam, mNandDataSize, mNandSpareSize>::quarticElp(int32_t sigmaN[])
{
    //****************************************************************
    //  Function:   quarticElp
    //
...
template <uint32_t mMParam, uint32_t mTParam, uint32_t mNandDataSize, uint32_t mNandSpareSize>
bool
NandBCHRTime<mMParam, mTParam, mNandDataSize, mNandSpareSize>::rootFindChien(int32_t sigmaN[],
                                                                             const int32_t LnOrig)
{
    //****************************************************************
...
template <uint32_t mMParam, uint32_t mTParam, uint32_t mNandDataSize, uint32_t mNandSpareSize>
bool
NandBCHRTime<mMParam, mTParam, mNandDataSize, mNandSpareSize>::fixErrors(int32_t Ln)
{
    //****************************************************************
    //  Function: fixErrors
    //
