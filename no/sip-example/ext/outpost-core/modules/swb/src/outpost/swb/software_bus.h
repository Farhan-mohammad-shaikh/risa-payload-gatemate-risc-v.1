...
public:
     ...
     */
    SoftwareBus(container::SharedBufferPoolBase& pool,
     ...
     */
    OperationResult
    sendMessage(const IDType id, const Slice<const uint8_t>& data);
...
protected:
     ...
     */
    virtual bool
    valid(IDType, const Slice<const uint8_t>&)
    {
        return true;
    }
...
private:
    OperationResult
    copySliceToBuffer(const Slice<const uint8_t>& slice,
                      container::ConstSharedBufferPointer& buffer);

    rtos::Mutex mMutex;

    container::SharedBufferPoolBase& mPool;
    container::ReferenceQueueBase<Message<IDType>>& mQueue;
 ...
 *               operator()(const IDType&, const Slice<const uint8_t>&) that returns true
...
{
public:
    SoftwareBusFiltered(container::SharedBufferPoolBase& pool,
                        container::ReferenceQueueBase<Message<IDType>>& queue,
                        uint8_t priority,
                        support::parameter::HeartbeatSource heartbeatSource) :
        SoftwareBus<IDType>(pool, queue, priority, heartbeatSource)
    {
        static_assert(std::is_convertible<Filter*, swb::MessageFilter<IDType>*>::value,
...
protected:
    bool
    valid(IDType id, const Slice<const uint8_t>& buffer) override
    {
