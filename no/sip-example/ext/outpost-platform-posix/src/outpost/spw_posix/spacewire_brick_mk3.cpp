...
SpacewireBrickMk3::SpacewireBrickMk3(const char* serialNumber) :
    StardundeeSpacewire(serialNumber, DeviceType::BRICK_MK3)
{
}

bool
SpacewireBrickMk3::isValid()
{
...
bool
SpacewireBrickMk3::configure()
{
    return configureLinkSpeed();
}

bool
SpacewireBrickMk3::configureLinkSpeed()
{
    bool result = false;
    BrickMk3LinkPort* linkPort = getLinkPort(mPort);

    stardundee::com::starsystem::deviceconfig::mk2devices::pcie::ClockRateParams params;
    if (linkPort != nullptr && 1 == linkPort->GetTransmitClock(params))
    {
        switch (mLinkSpeed)
        {
...
bool
SpacewireBrickMk3::up(outpost::time::Duration timeout)
{
    bool result = false;
    BrickMk3LinkPort* linkPort = getLinkPort(mPort);
...
void
SpacewireBrickMk3::down(outpost::time::Duration)
{
    BrickMk3LinkPort* linkPort = getLinkPort(mPort);
    if (linkPort != nullptr)
    {
...
void
SpacewireBrickMk3::flushReceiveBuffer()
{
}

SpacewireBrickMk3::~SpacewireBrickMk3()
{
    this->close();
    delete mConfiguration;
}

stardundee::com::starsystem::deviceconfig::mk2devices::brickmk3::BrickMk3LinkPort*
SpacewireBrickMk3::getLinkPort(uint8_t port)
{
    if (isValid())
    {
        return dynamic_cast<
