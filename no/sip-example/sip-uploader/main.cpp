...
int main(int argc, char** argv)
{
	...
	try
	{
		...
		std::cout << "Filename: ";
		std::cin >> filename;
		std::cout << "Memory Address in Decimal: ";
		...
		uint8_t cur_byte;
		while (fin.read(reinterpret_cast<char*>(&cur_byte), sizeof(cur_byte))) 
		{
			filebytes.push_back(cur_byte);
		}
		fin.close();

		startDataWrite(filebytes, memory_address, true);
		sendAllData(filebytes);

		#if 0
		const size_t chunkSize = 10000;
		size_t totalBytes = filebytes.size();
		size_t offset = 0;

		std::cout << "offset: " << offset << " totalbytes: " << totalBytes << std::endl;
		while (offset < totalBytes) 
		{
			size_t bytesToSend = std::min(chunkSize, totalBytes - offset);
			std::vector<uint8_t> chunk(filebytes.begin() + offset, filebytes.begin() + offset + bytesToSend);

			startDataWrite(chunk, memory_address, true);
			sendAllData(chunk);

			offset += bytesToSend;
			memory_address += bytesToSend;
		}
		#endif
...
void startDataWrite(std::vector<uint8_t> &data, uint32_t address, bool write_flash)
{
	...
	uint8_t payload[] = {write_flash, static_cast<uint8_t>((address & 0xFF)), static_cast<uint8_t>((address >> 8)), 
	static_cast<uint8_t>((address >> 16)), static_cast<uint8_t>(data.size()), static_cast<uint8_t>((data.size() >> 8)), static_cast<uint8_t>((data.size() >> 16))};
